...
<nav class="navbar bs-docs-nav" role="navigation" style="margin-top:10px;">
  ...
  <div class="container"> 
         <div class="row">
			 ...
			 <ul class="list-unstyled left-border">
   <?php if(is_array($foodcatlist)): $i = 0; $__LIST__ = array_slice($foodcatlist,0,7,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub): $mod = ($i % 2 );++$i;?><li> <a href="<?php echo U('List/index/','id='.$sub['fcid']);?>" class="list-group-item"><span style="margin-left:60px;"><?php echo ($sub["fcname"]); ?></span><span class="fa fa-caret-right pull-right"></span></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
			...
			<div class="row top5" >
				  <?php if(is_array($foodlist)): $i = 0; $__LIST__ = array_slice($foodlist,0,9,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="col-sm-4 col-xs-6 simpleCart_shelfItem row-padding">
			   <div class="thumbnail i_list">
		<?php if(($islogin) == "1"): ?><a data-toggle="modal" href="<?php echo U('Index/show','id='.$vo['fid']);?>" data-target="#modal">

		<?php else: ?>
		<a href="<?php echo U('Product/index','id='.$vo['fid']);?>" ><?php endif; ?>
	  <img  src="<?php echo ($url); echo ($vo["fpic"]); ?>">
	   </a>
	  <div class="caption">
        <h3 class="item_name"><?php echo ($vo["fname"]); ?></h3>
        <input value="1" class="item_Quantity" type="hidden">
    <p ><span class="pull-left item_price" style="color:#FF7700;font-weight:600;font-size:14px"> ￥<?php echo ($vo["fprice"]); ?> </span>
...
<?php if(empty($uid)): if(($isorder) == "0"): ?><a rel="nofollow" class="btn btn-warning" href="/index.php?m=public&a=login&reurl=<?php echo urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']); ?>" ><i class="fa fa-shopping-cart"></i> 立刻结算</a>
		...
		<a id="cartSubmit" data-toggle="modal" data-target="#myModal" class="btn btn-warning"><i class="fa fa-shopping-cart"></i> 立刻结算</a><?php endif; ?>
	
	<?php else: ?> 
	<a href="<?php echo U('Cart/index/');?>" class="btn btn-warning"><i class="fa fa-shopping-cart"></i> 立刻结算</a><?php endif; ?>	
 </p>
 	  </div>
    </div>
  </div>	 
...
<ul class="list-unstyled gg" style="margin-bottom:0px;">

<?php if(is_array($toplist)): $i = 0; $__LIST__ = $toplist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('Article/s/','id='.$vo['aid']);?>" title="<?php echo ($vo["atitle"]); ?>"><?php echo ($vo["atitle"]); ?></a> </li><?php endforeach; endif; else: echo "" ;endif; ?>
</ul>
...
<div class="container">
	<?php if(empty($llist)): else: ?> 友情链接：<?php if(is_array($llist)): $i = 0; $__LIST__ = $llist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a style="color:#aaa" href="<?php echo ($vo["link"]); ?>" target="_blank"> <?php echo ($vo["linkname"]); ?> </a> 　　<?php endforeach; endif; else: echo "" ;endif; endif; ?>
