...
<head>
	<meta charset="utf-8" />
	<title>积分兑换_<?php echo ($name); ?></title>
  ...
  <?php if(is_array($foodcatlist)): $i = 0; $__LIST__ = array_slice($foodcatlist,0,7,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub): $mod = ($i % 2 );++$i;?><li> <a href="<?php echo U('List/index','id='.$sub['fcid']);?>" class="list-group-item"><span style="margin-left:60px;"><?php echo ($sub["fcname"]); ?></span><span class="fa fa-caret-right pull-right"></span></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
			 ...
			 <div class="col-md-9 ">
			  ...
			  <div class="row" style="margin-top:10px;min-height:500px;">
	             <?php if(is_array($giftlist)): $i = 0; $__LIST__ = $giftlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="col-md-3">
			   <div class="thumbnail i_lists">
	
	  <img  src="<?php echo ($vo["goodpic"]); ?>">
	  
	  <div class="">
        <h3><?php echo ($vo["goodname"]); ?></h3>
       
    <p>积分：<span style="color:#FF7700;font-weight:600;font-size:14px" ><?php echo ($vo["credits"]); ?></span>　　　
	<?php if(empty($uid)): ?><a rel="nofollow" class="btn btn-danger btn-xs" href="/index.php?m=public&a=login&reurl=<?php echo urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']); ?>" > 我要兑换</a>
	
	<?php else: ?>
	<a class="btn btn-danger btn-xs "  href="<?php echo U('Gift/add/','id='.$vo['cgid']);?>" data-toggle="modal" data-target="#myModals"> 我要兑换</a></p><?php endif; ?>
